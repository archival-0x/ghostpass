...
func NewField(key *memguard.Enclave, username string, pwd *memguard.Enclave) (*Field, error) {
...
func (f *Field) DecryptFieldSecret(enclave *memguard.Enclave) (string, string, error) {
	plaintext, err := BoxDecrypt(enclave, []byte(f.Secret))
	if err != nil {
		return "", "", err
	}
